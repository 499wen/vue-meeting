...
export default {
  ...
  methods: {
    ...
    submit(val, key) {
      ...
      // console.log(this.formData) ${this.externalCode}
    ...
    onDrag: function (e) {
      ...
      if (this.imgs && Object.keys(this.imgs).length === 0) {
        this.$refs.dropbox.style = "border:2px dashed #007bff;";
      } else {
        this.$refs.dropbox.style = " ";
      }
    },
    onDragLeave: function (e) {
      ...
      e.preventDefault();
      console.log("离开");
      this.$refs.dropbox.style = "border:2px dashed #ddd;";
    },
    onDrop: function (e) {
      e.stopPropagation();
      e.preventDefault();
      console.log("松手");
      this.fil = e.dataTransfer.files;
      // console.log('dt',this.fil)
      for (let i = 0; i < this.fil.length; i++) {
        let size = Math.floor(this.fil[i].size / 1024);
        if (size > 2 * 1024 * 1024) {
          // alert('请选择2M以内的图片！');
          this.$message.warning('请选择2M以内的图片！')
          return false
        }
  ...
  mounted() {
    ...
    // var file = document.querySelector('#exportExcelFile')
    // file.addEventListener("focus",function () {
    //   console.log("focus", file.files)
    // })
